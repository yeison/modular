...
When recursing through directories, `mojo test` automatically ignores any
directories whose names begin with a dot (such as `.git`, `.vscode`, or
`.pytest_cache`).
...
You can also produce JSON output for test collection. Consider the [example
testing
project](https://github.com/modular/modular/tree/main/examples/mojo/testing)
directory structure shown in the [Running tests](#running-tests) section. The
