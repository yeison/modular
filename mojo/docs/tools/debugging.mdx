...
Both the `mojo` and `modular` [packages](/max/packages/) include the [LLDB
debugger](https://lldb.llvm.org/) and a Mojo LLDB plugin. Together these
provide the low-level debugging interface for the Mojo extension. You can also
use the `mojo debug` command to start a command-line debugging session using
LLDB or to launch a Mojo debugging session in VS Code.

You can also debug Mojo programs on GPUs, but it requires some extra software
and configuration. Currently GPU debugging only works with NVIDIA GPUs. For
details, see [GPU debugging](/mojo/tools/gpu-debugging).
