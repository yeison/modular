...
for item in list:     # Copies the integer into "item"
      print(item, end=", ")
      i = 0 # warning: assignment to 'i' was never used
...
You can also subscript the list with an integer:

```mojo
#: from collections import List
var list = [2, 3, 4]
for ref item in list:     # Captures reference into list.
      print(item, end=", ")
      item = 0 # mutates the entry in the list itself
print("\nAfter loop:", list[0], list[1], list[2])
```
