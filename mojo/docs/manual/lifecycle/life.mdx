...
ownership](/mojo/manual/values/ownership#transfer-arguments-owned-and-) of a
...
(`read`, `mut`, and `owned`)—the only catch is that you must use the `^`
transfer sigil to end the lifetime of a move-only type when assigning it to a
new variable or when passing it as an `owned` argument. The
