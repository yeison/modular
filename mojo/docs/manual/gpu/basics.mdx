...
And open `basics.mdx` to run the code cells interactively. If you don't have `mojo` on your system PATH it will automacailly download and install it the first time you run a Mojo cell.
...
def main():
    ...
    ctx.call_function[printing_kernel](grid_dim=1, block_dim=4)
    ctx.synchronize()
```
...
We can pass this function as a parameter to `call_function()` to compile it
...
```mojo :once
ctx.call_function[printing_kernel](grid_dim=1, block_dim=(2, 2, 2))
ctx.synchronize()
```
...
ctx.call_function[block_kernel](grid_dim=(2, 2), block_dim=2)
ctx.synchronize()
```
...
var in_buffer = ctx.create_buffer[dtype](elements_in)
```
...
ctx.call_function[print_values_kernel](
...
ctx.call_function[multiply_kernel[2]](
    in_tensor,
    grid_dim=blocks,
    block_dim=threads,
)
...
```mojo :clear
var out_buffer = ctx.create_buffer[dtype](blocks)
...
ctx.call_function[sum_reduce_kernel](
    in_tensor,
    out_tensor,
    grid_dim=blocks,
    block_dim=threads,
...
ctx.call_function[simd_reduce_kernel](
    in_tensor,
    out_tensor,
    grid_dim=blocks,
...
ctx.call_function[warp_reduce_kernel](
    in_tensor,
    out_tensor,
    grid_dim=blocks,
    block_dim=threads,
...
elements, don't forget to zero the values with `memset()`.
