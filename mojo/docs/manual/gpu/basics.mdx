...
fn dummy_kernel(buffer: DeviceBuffer[dtype_u8]):
...
fn sum_reduce_kernel(
    in_tensor: InputLayoutTensor, out_buffer: DeviceBuffer[dtype]
):
...
fn simd_reduce_kernel(
    ...
    out_buffer[block_idx.x] = in_tensor.load[4](block_idx.x, 0).reduce_add()
...
fn reduce_sum_f32(in_tensor: InputTensorF32, output_buffer: DeviceBuffer[dtype_f32]):
    var value = in_tensor.load[1](block_idx.x, thread_idx.x)
    value = warp.sum(value)
    if thread_idx.x == 0:
...
```text
DeviceBuffer([6.0, 22.0, 38.0, 54.0, 70.0, 86.0, 102.0, 118.0])
```
