...
struct EngineTensor(Sized):
    ...
    fn buffer[type: DType](self) raises -> Buffer[Dim(), type]:
        return Buffer[Dim(), type](self.data[type](), len(self))

    fn buffer(self) -> Buffer[Dim(), DType.invalid]:
        return Buffer[Dim(), DType.invalid](
