...
fn print_svg(tensor_base: LayoutTensor, tensor: LayoutTensor) raises:
    ...
    debug_assert(tensor.layout.rank() == 2, "Layout rank must be 2")

    debug_assert(
        tensor.layout.shape[0].value() <= tensor_base.layout.shape[0].value(),
        "Layout 0 should have the largest first dimension",
    )
    debug_assert(
        tensor.layout.shape[1].value() <= tensor_base.layout.shape[1].value(),
    ...
    if tensor.element_layout.rank() == 2:
        for i in range(tensor.layout.shape[0].value()):
            for j in range(tensor.layout.shape[1].value()):
                for e_i in range(tensor.element_layout.shape[0].value()):
                    for e_j in range(tensor.element_layout.shape[1].value()):
                        var offset = (
        ...
        for i in range(tensor.layout.shape[0].value()):
            for j in range(tensor.layout.shape[1].value()):
                var offset = (Int(tensor.ptr) - Int(tensor_base.ptr)) // sizeof[
                    Scalar[tensor.dtype]
                ]()
                var idx = tensor.layout(IntTuple(i, j)) + offset
                var orig_pos = map[idx]
                var x = margin + text_margin + orig_pos[1].value() * cell_size
                var y = start_y + orig_pos[0].value() * cell_size
                print(
                    '<rect x="',
                    x,
