...
fn elementwise_epilogue_c_tile[
...
](offset: GemmShape, tile_len: GemmShape, c: NDBuffer[type, 2, shape_c]):
    @always_inline
    @parameter
    fn activation_on_col_chunk[col_chunk_size: Int](idx_n: Int):
        ...
        for idx_m in range(tile_len.M):
            ...
            var c_val = c.load[col_chunk_size](c_coord)
...
struct PackMatrixRows[
    ...
    fn _transpose_pack_helper[
        ...
        fn body[idx: Int]():
            ...
            if skip_row_bound or (inner_row_idx < read_bound[0]):
                ...
                if skip_col_bound:
                    ...
                    row_data = self.original_matrix.load[simd_size](
                        row_global_index
                    )
                else:
                ...
                transpose_buffer.store[simd_size]((inner_row_idx, 0), row_data)
            else:
                # Row out of defined bound, fill the transpose buffer with zero
                transpose_buffer.store[simd_size](
        ...
        fn transposed_inner_row_body[idx: Int]():
            var transposed_data = transpose_buffer.load[simd_size]((idx, 0))
            ...
            if skip_col_bound or (idx < write_bound[1]):
                self.packed_matrix.store[simd_size](
                    (
                        _row_outer,
                        local_off_set[1] + idx,
                        _row_inner,
    ...
    fn _pack_helper[
        ...
        fn pack_vector(row_idx: Int, col_idx: Int):
            ...
            if skip_col_bound or (
                ...
                data = self.original_matrix.load[simd_size](global_idx)
        ...
        for row in range(tile_rows):
            ...
            for col in range(
                ...
                if is_load:
                    ...
                    var data = self.output_tile.load[column_step](
                        Index(row, col)
                    )
...
struct MatmulInnerLoopBPacked[
    ...
    var a: NDBuffer[a_type, 2, shape_a]
    ...
    fn _store_c_tile(
        ...
        if self.use_i8mm:
            ...
            @parameter
            fn body_i8mm[idx0: Int, idx1: Int]():
                var c_data = c_local.load[simd_size](
                    Index(idx0, idx1 * simd_size)
                )
                if skip_boundary_check or (
                    idx1 * 2 + 2 <= self.c_bound[1] - tile_n_idx
        ...
        @parameter
        fn body[idx0: Int, idx1: Int]():
            var c_data = c_local.load[simd_size](Index(idx0, idx1 * simd_size))
            if skip_boundary_check or (
                idx1 * simd_size + simd_size <= self.c_bound[1] - tile_n_idx
            ):
    ...
    fn _accumulate_lane[
        ...
        for row in range(a_row_size):
            ...
            var a_val = self.a.load[a_col_size](global_m, global_k).cast[
                c_type
            ]()
            a_vals[row] = a_val
        ...
        for lane in range(a_col_size):
            ...
            for col in range(pack_inner_size // simd_size):
                ...
                for row in range(a_row_size):
                    ...
                    var c_val = c_local.load[simd_size](c_idx)
                    c_val = fma[c_type, simd_size](a_val[lane], b_val, c_val)
                    c_local.store[simd_size](c_idx, c_val)
...
struct TiledMatmul[
    ...
    var c: NDBuffer[c_type, 2, config.shape_c]
    var a: NDBuffer[a_type, 2, config.shape_a]
    var b: NDBuffer[b_type, 2, config.shape_b]
    ...
    @staticmethod
    fn run(
        c: NDBuffer[c_type, 2, config.shape_c],
        a: NDBuffer[a_type, 2, config.shape_a],
        b: NDBuffer[b_type, 2, config.shape_b],
...
struct BTileGenerator[
    ...
    @staticmethod
    fn get(
        b: NDBuffer[type, 2, config.shape_b], tile_n_k: StaticIntTuple[2]
    ...
    if transpose_b:
            ...
            for n in range(N):
                ...
                fn compute_fn[width: Int](k: Int):
                    ...
                    if width == 1:
                        ...
                        acc_vector += a.load[simd_width](m, k) * b.load[
                            simd_width
                        ](n, k)
        ...
        fn last_update[
            ...
            if epilogue_wrapper:
                ...
                c.store[width](coords, rebind[SIMD[type, width]](val))
        ...
        fn accum_out_row[
            ...
            fn _wrapper[simd_width: Int](n: Int):
                output_func[type, simd_width](
                    Index(m, n),
                    c.load[simd_width](m, n) + a_val * b.load[simd_width](k, n),
                )
        ...
        for w_sub_col_idx in range(WNITER):
            ...
            for res_idx_m in range(TM):
                ...
                for res_idx_n in range(0, int(TN), 4):
                    ...
                    var result_vec = thread_results.load[4](
                        Index(
                            w_sub_row_idx,
                            w_sub_col_idx,
                            res_idx_m,
...
fn _submatmul_sequential_sync[
    ...
    fn dispatch_on_kernel_type[kernel_type: Bool]():
        ...
        fn elementwise_closure(offset: GemmShape, shape: GemmShape):
            ...
            if elementwise_lambda_fn:
                elementwise_epilogue_c_tile[
                    ...
                    offset,
                    shape,
                    rebind[NDBuffer[c_type, 2, mm_config.shape_c]](c),
                )
            else:
                pass
        ...
        TiledMatmul[
            ...
            rebind[NDBuffer[c_type, 2, mm_config.shape_c]](c),
            rebind[NDBuffer[a_type, 2, mm_config.shape_a]](a),
            rebind[NDBuffer[b_type, 2, mm_config.shape_b]](b),
            elementwise_closure,
            rowwise_epilogue_fn,
            sub_matrix_shape,
            sub_matrix_offset,
