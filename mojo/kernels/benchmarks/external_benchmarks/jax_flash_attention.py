...
def build_kernel(
    ...
    def kernel(
        ...
        q_barrier = arith.addi(
            c(blocks.stages), arith.index_cast(index, wg_idx)
        )
        ...
        def kv_loop(kv_step, carry):
            ...
            with only_wg(0), ctx.named_region("KV TMA wait"):
                ...
                with ir.InsertionPoint(
                    scf.IfOp(wait_step_in_bounds).then_block
                ):
...
def benchmark_and_verify(
    ...
    **kwargs,
    ...
    with mlir.make_ir_context(), ir.Location.unknown():
        ...
        q = random.normal(
            kq,
            (batch_size, num_q_heads, q_seq_len, head_dim),
            dtype=jnp.float16,
        )
        k = random.normal(
            kk,
...
if __name__ == "__main__":
    ...
    for kv_seq_len, q_seq_len, head_dim in problem_it:
        ...
        for exp_impl, block_q, block_kv in param_it:
            ...
            print(
                f"exp_impl={exp_impl.name:<6} block_q={block_q:<4}block_kv={block_kv:<4}:"
                f"  {runtime_us:<7.1f}us = {achieved_tc_util:4.1f}% TC"
                " utilization"
            )
