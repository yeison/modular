...
fn matmul(
    ...
    let a = NDBuffer[2, DimList.create_unknown[2](), DType.index](
        a_ptr, Index(m, k)
    )
    let b = NDBuffer[2, DimList.create_unknown[2](), DType.index](
        b_ptr, Index(k, n)
    )
    let c = NDBuffer[2, DimList.create_unknown[2](), DType.index](
        c_ptr, Index(m, n)
    )
    ...
    let c_reg = stack_allocation[TILE_SZ_B, DType.index]()
    ...
    for tile_idx in range((k - 1) // TILE_SZ_RATIO + 1):
        ...
        let b_val = b[tile_idx * TILE_SZ_RATIO + i, col + j].to_int()
        ...
        for idx in range(TILE_SZ_RATIO):
            ...
            if row < m and tile_idx * TILE_SZ_RATIO + idx < k:
                a_reg = a[row, tile_idx * TILE_SZ_RATIO + idx].to_int()
            else:
                a_reg = 0
...
fn run_matmul() raises:
    ...
    let a_device = _malloc[DType.index](m * k)
    let b_device = _malloc[DType.index](k * n)
    let c_device = _malloc[DType.index](m * n)
