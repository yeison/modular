...
fn test_pad_constant_gpu[
    ...
    paddings: Buffer[DType.index, 2 * rank],
    ctx: DeviceContext,
    verbose: Bool = False,
) raises:
    ...
    ctx.enqueue_copy_to_device(in_device, input.data)
    ctx.enqueue_copy_to_device(out_device, output.data)

    # pad with constant = 5
    var constant = Scalar[type](5)

    pad_constant(
        out_device.unsafe_pointer(),
        output_shape,
        in_device.unsafe_pointer(),
        input_shape,
        paddings.data,
        constant,
        ctx,
    ...
    ctx.enqueue_copy_from_device(output.data, out_device)
    ctx.synchronize()
...
def main():
    ...
    with DeviceContext() as ctx:
        ...
        var paddings_1d = Buffer[DType.index, 2 * 1].stack_allocation()
        paddings_1d[0] = 2  # axis-0 pre-pad
        paddings_1d[1] = 1  # axis-0 post-pad
        test_pad_constant_gpu[type](input_shape_1d, paddings_1d, ctx)
        # CHECK: PASS: rank=1

        var input_shape_2d = IndexList[2](32, 32)
        # Create a padding array of the (before,after) form
        var paddings_2d = Buffer[DType.index, 2 * 2].stack_allocation()
        ...
        # Create a padding array of the (before,after) form
        var paddings_3d = Buffer[DType.index, 2 * 3].stack_allocation()
