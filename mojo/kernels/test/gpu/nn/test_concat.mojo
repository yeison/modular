...
fn test_concat_4_inputs_rank5() raises:
    ...
    let input_shape = DimList(d0, d1, d2, d3, d4)
    let output_shape = DimList(d0, d1, d2, d3, 4)

    let input_0_host = _create_buffer_host[rank, dtype](input_shape)
    let input_1_host = _create_buffer_host[rank, dtype](input_shape)
    let input_2_host = _create_buffer_host[rank, dtype](input_shape)
    let input_3_host = _create_buffer_host[rank, dtype](input_shape)
    ...
    let input_0_device = _create_buffer_device[rank, dtype](input_shape)
    let input_1_device = _create_buffer_device[rank, dtype](input_shape)
    let input_2_device = _create_buffer_device[rank, dtype](input_shape)
    let input_3_device = _create_buffer_device[rank, dtype](input_shape)
    ...
    let output_device = _create_buffer_device[rank, dtype](output_shape)
    ...
    let output_host = _create_buffer_host[rank, dtype](output_shape)
    ...
    fn validate_results():
        ...
        for i in range(d0):
            for j in range(d1):
                for k in range(d2):
                    for l in range(d3):
                        let not_match_0 = output_host[
                            i, j, k, l, 0
                        ] != input_0_host[i, j, k, l, 0]
                        let not_match_1 = output_host[
                            i, j, k, l, 1
                        ] != input_1_host[i, j, k, l, 0]
                        let not_match_2 = output_host[
                            i, j, k, l, 2
                        ] != input_2_host[i, j, k, l, 0]
                        let not_match_3 = output_host[
                            i, j, k, l, 3
