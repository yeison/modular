...
fn run_matmul() raises:
    ...
    _copy_host_to_device(a_device, a_host.data().as_scalar_pointer(), m * k)
    _copy_host_to_device(b_device, b_host.data().as_scalar_pointer(), k * n)
    ...
    _copy_device_to_host(
        c_host.data(), DTypePointer[DType.float32](c_device.address), m * n
    )
