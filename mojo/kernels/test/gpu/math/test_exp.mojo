...
fn run_elementwise() raises:
    ...
    let in_buffer = NDBuffer[1, DimList.create_unknown[1](), DType.float32](
        in_device, Index(length)
    )
    let out_buffer = NDBuffer[1, DimList.create_unknown[1](), DType.float32](
        out_device, Index(length)
    )

    @always_inline
    @parameter
    fn func[simd_width: Int, rank: Int](idx0: StaticIntTuple[rank]):
        let idx = rebind[StaticIntTuple[1]](idx0)
