...
fn matmul_naive[
    ...
    C: LayoutTensor[elt, layoutC],
    A: LayoutTensor[elt, layoutA],
    B: LayoutTensor[elt, layoutB],
):
...
fn matmul_ukern[
    ...
    C: UnsafePointer[Scalar[elt], 0, 0, 64],
    A: UnsafePointer[Scalar[elt]],
    B: UnsafePointer[Scalar[elt], 0, 0, 64],
    inc: Bool,
):
    ...
    var Bo: UnsafePointer[Scalar[elt], 0, 0, 64] = B
...
fn matmul[
    ...
    alias Ptr = UnsafePointer[Scalar[elt], 0, 0, 64]
    ...
    for _ in range(M // Mc):
        var pb: UnsafePointer[Scalar[elt], 0, 0, 64] = B.ptr
        var pak: __type_of(pb) = pa
        for _ in range(N // Nc):
            var pck: UnsafePointer[Scalar[elt], 0, 0, 64] = pc
            pak = pa
            for kc in range(K // Kc):
                var pbk: UnsafePointer[Scalar[elt], 0, 0, 64] = pb
                pck = pc
...
fn alloc_tensor[elt: DType, layout: Layout]() -> LayoutTensor[elt, layout]:
    return LayoutTensor[elt, layout](
        UnsafePointer[Scalar[elt], 0, 0, 64].alloc(layout.size())
    )
...
fn alloc_tensor[
    ...
    return LayoutTensor[elt, layout](
        UnsafePointer[Scalar[elt], 0, 0, 64].alloc(rtlayout.size()), rtlayout
    )
...
fn vectorize_layout_tensor[
    ...
    unroll_factor: Int = 4,
](a: LayoutTensor[elt_a, layout_a], b: LayoutTensor[elt_b, layout_b],):
...
fn copy_to[
    ...
    src: LayoutTensor[elt_src, layout_src],
):
    @always_inline
    @parameter
    fn copy[
...
fn matmulb2b[
    ...
    var AB: UnsafePointer[Scalar[elt], 0, 0, 64] = stack_allocation[
    ...
    for _ in range(M // Mc):  # mc
        var pb: UnsafePointer[Scalar[elt], 0, 0, 64] = B.ptr
        var pc: UnsafePointer[Scalar[elt], 0, 0, 64] = C.ptr
        var pak: UnsafePointer[Scalar[elt], 0, 0, 64] = pa
        var pdk: UnsafePointer[Scalar[elt], 0, 0, 64] = pd
        for lc in range(L // Nc):  # lc, reduction for (AB)*C
            ...
            for kc in range(
                ...
                var pabk: UnsafePointer[Scalar[elt], 0, 0, 64] = AB
                var pbk: UnsafePointer[Scalar[elt], 0, 0, 64] = pb
