...
@value
struct amd_signal_kind_t:
    alias AMD_SIGNAL_KIND_INVALID = 0
    alias AMD_SIGNAL_KIND_USER = 1
    alias AMD_SIGNAL_KIND_DOORBELL = -1
    alias AMD_SIGNAL_KIND_LEGACY_DOORBELL = -2
...
struct amd_signal_t:
    ...
    var reserved3: StaticTuple[UInt32, 2]
...
fn hsa_signal_add(sig: hsa_signal_t, value: UInt64):
    var s = UnsafePointer.address_of(sig.handle).bitcast[
...
fn send_signal(signal: UInt64):
    hsa_signal_add(hsa_signal_t(signal), 1)
...
struct Header:
    ...
    @staticmethod
    fn fill_packet(
        self: UnsafePointer[Self, address_space=1],
        payload: UnsafePointer[Payload, address_space=1],
        service_id: UInt32,
        arg0: UInt64,
        arg1: UInt64,
        arg2: UInt64,
        ...
        payload[].slots[int(me)][0] = arg0
        payload[].slots[int(me)][1] = arg1
        payload[].slots[int(me)][2] = arg2
        payload[].slots[int(me)][3] = arg3
        payload[].slots[int(me)][4] = arg4
        payload[].slots[int(me)][5] = arg5
        payload[].slots[int(me)][6] = arg6
        payload[].slots[int(me)][7] = arg7
    ...
    fn get_return_value(
        ...
        while True:
            ...
            if me == low:
                var ptr = UnsafePointer.address_of(self[].control)
...
@register_passable("trivial")
struct Buffer:
    var headers: UnsafePointer[Header, address_space=1]
    var payloads: UnsafePointer[Payload, address_space=1]
    var doorbell: UInt64
    ...
    fn get_header(
        self: UnsafePointer[Self, address_space=1],
        ptr: UInt64,
    ) -> UnsafePointer[Header, address_space=1]:
        return self[].headers.offset(ptr & self[].index_mask)

    @staticmethod
    @always_inline
    fn get_payload(
        self: UnsafePointer[Self, address_space=1], ptr: UInt64
    ) -> UnsafePointer[Payload, address_space=1]:
        return self[].payloads.offset(ptr & self[].index_mask)

    @staticmethod
    fn pop(
        self: UnsafePointer[Self, address_space=1],
        top: UnsafePointer[UInt64, address_space=1],
    ) -> UInt64:
        ...
        # one packet.
        while True:
            var p = Self.get_header(
                self,
                f,
            )
            var n = Atomic._fetch_add(
                UnsafePointer.address_of(p[].next),
                0,
            )
    ...
    fn pop_free_stack(
        ...
        if me == low:
            packet_ptr = Self.pop(
                self,
                UnsafePointer.address_of(self[].free_stack),
            )
    ...
    fn push(
        ...
        var f = Atomic._fetch_add(top, 0)
        var p = Self.get_header(self, ptr)
        while True:
            p[].next = f
    ...
    fn push_ready_stack(
        ...
        if me == low:
            Self.push(
                self,
                UnsafePointer.address_of(self[].ready_stack),
                ptr,
            )
            send_signal(self[].doorbell)

    @staticmethod
    fn return_free_packet(
        self: UnsafePointer[Self, address_space=1],
        ptr: UInt64,
        me: UInt32,
        low: UInt32,
        ...
        """
        if me == low:
            var ptr = inc_ptr_tag(ptr, self[].index_mask)
            Self.push(self, UnsafePointer.address_of(self[].free_stack), ptr)
...
fn _hostcall(
    buffer: UnsafePointer[Buffer, address_space=1],
    service_id: UInt32,
    arg0: UInt64,
    arg1: UInt64,
    arg2: UInt64,
    ...
    *** INTERNAL USE ONLY ***
    ...
    var packet_ptr = Buffer.pop_free_stack(buffer, me, low)

    var header = Buffer.get_header(buffer, packet_ptr)
    var payload = Buffer.get_payload(buffer, packet_ptr)
    ...
    Buffer.push_ready_stack(buffer, packet_ptr, me, low)

    var retval = Header.get_return_value(header, payload, me, low)
    Buffer.return_free_packet(buffer, packet_ptr, me, low)
    return retval
...
fn hostcall(
    ...
    *** PREVIEW FEATURE ***
    ...
    """
    var addr = implicitarg_ptr().bitcast[UInt64]().offset(10)[]
    var ptr = UnsafePointer.address_of(addr).bitcast[
        UnsafePointer[UInt64, address_space=1]
    ]()[]
    var buffer = ptr.bitcast[Buffer]()
...
fn message_append_bytes(
    service_id: UInt32, msg_desc: UInt64, mut data: Span[UInt8]
) -> (UInt64, UInt64):
    """
...
@value
struct FprintfCtrl:
    alias stdout = UInt32(0)
    alias stderr = UInt32(1)
...
fn fprintf_append_string_n(
    msg_desc: UInt64, mut data: Span[UInt8], is_last: Bool
) -> UInt64:
    """
...
fn print():
    ...
    # prints 32
    var s = "32".as_bytes()
    _ = printf_append_string_n(msg, s, is_last=1)
