...
convert Python values to Mojo `Bool` and `Float64` types using the
[`bool()`](/mojo/stdlib/builtin/bool/bool-function) and
[`float()`](/mojo/stdlib/builtin/floatable/float/) built-in functions, construct
an `Int` using the [`Int()`](/mojo/stdlib/builtin/int/Int/#__init__)
constructor, and construct a `String` using the
[`String()`](/mojo/stdlib/collections/string/String/#__init__) constructor.
`PythonObject` also conforms to the
