...
You don't need a `main()` function when coding in the [REPL](/mojo/cli/repl) or
in a Jupyter notebook. In these environments, Mojo executes top-level code (that
is, statements that appear outside of a function). Mojo doesn't support
top-level code in a `.mojo` (or `.🔥`) file.
