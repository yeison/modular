...
[REPL](/mojo/cli/repl) or in a
[Jupyter
notebook](https://github.com/modular/mojo/tree/main/examples/notebooks#readme).
In these environments, Mojo executes top-level code (that is, statements
that appear outside of a function). Mojo doesn't support top-level code in a
`.mojo` (or `.🔥`) file
