...
def test_weights_format__raises_with_no_weights_path():
    with pytest.raises(ValueError):
        weights_format([])


def test_weights_format__raises_with_bad_weights_path():
    with pytest.raises(ValueError):
        weights_format([Path("this_is_a_random_weight_path_without_extension")])


def test_weights_format__raises_with_conflicting_weights_path():
    with pytest.raises(ValueError):
        weights_format(
            [
...
def test_weights_format__correct_weights_format():
