...
from max.graph import core as _c, DType
...
def test_tensor_type(mlir_context) -> None:
    ...
    dtype = _c.dtype_type(mlir_context, "f32")
    dim1 = _c.static_dim(mlir_context, 3)
    dim2 = _c.symbolic_dim(mlir_context, "x")
    tensor_type = _c.tensor_type(mlir_context, dtype, [dim1, dim2])
...
def test_tensor_type_accessors(mlir_context) -> None:
    ...
    assert _c.tensor_type_get_dtype(tensor_type) == DType.float32._mlir
    assert _c.tensor_type_get_rank(tensor_type) == 2


def test_opaque_type(mlir_context) -> None:
    """Tests opaque type creation and properties."""
    opaque = _c.opaque_type(mlir_context, "custom_type")
    assert _c.type_is_opaque(opaque)
    assert not _c.type_is_tensor(opaque)
    assert _c.opaque_type_name(opaque) == "custom_type"


def test_type_checking(mlir_context) -> None:
    """Tests type checking functions."""
    dtype = _c.dtype_type(mlir_context, "f32")
    dim = _c.static_dim(mlir_context, 3)
    tensor_type = _c.tensor_type(mlir_context, dtype, [dim])
    opaque = _c.opaque_type(mlir_context, "custom_type")

    assert _c.type_is_tensor(tensor_type)
    assert not _c.type_is_opaque(tensor_type)
    assert _c.type_is_opaque(opaque)
    assert not _c.type_is_tensor(opaque)
