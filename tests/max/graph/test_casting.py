...
def static_known_shape_size(shape: Shape):
    ...
    return math.prod(abs(dim) for dim in shape if isinstance(dim, int))
...
@given(
    input_type=tensor_types(shapes=st.shared(shapes(), key="shape")),
    output_shape=st.shared(shapes(), key="shape").flatmap(st.permutations),
)
...
@pytest.mark.skip("GRA-864")
@given(
    input_type=tensor_types(shapes=st.shared(shapes(), key="shape")),
    reshape_shape=negative_one_reshape(st.shared(shapes(), key="shape")),
)
...
def test_reshapes__zero_dim(input_type: TensorType, reshape_shape: list[Dim]):
    ...
    assume(  # TODO (MSDK-763)
        all(
            s in input_type.shape
            for s in reshape_shape
...
@given(
    input_type=tensor_types(shapes=st.shared(shapes(), key="shape")),
    reshape_shape=st.tuples(
        st.shared(shapes(), key="shape"), st.lists(st.just(1))
    ).map(lambda shape_ones: shape_ones[0] + shape_ones[1]),
)
...
@given(
    ...
    reshape_shape=st.shared(shapes(), key="shape"),
)
...
@given(
    input_type=st.shared(tensor_types(), key="input"),
    a=axes(st.shared(tensor_types(), key="input")),
    b=axes(st.shared(tensor_types(), key="input")),
)
...
# TODO: test squeeze and unsqueeze cases
@given(
