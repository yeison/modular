...
def test_rebind() -> None:
    ...
    with Graph(
        ...
        assert rebind_to_existing_names.shape == shape(("batch", "channels"))

        rebind_to_const = graph.inputs[1].rebind((3, 10))
        assert rebind_to_const.shape == shape((3, 10))

        rebind_to_new_names = graph.inputs[0].rebind(
            ("notbatch", "notchannels")
        )
        assert rebind_to_new_names.shape == shape(("notbatch", "notchannels"))
        ...
        assert rebind_expression.shape == shape(("batch", "expression"))
