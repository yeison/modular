...
def test_permute_success(input_type: TensorType, dims: list[int]):
    with Graph("permute", input_types=[input_type]) as graph:
...
def test_permute_out_of_range(input_type: TensorType, dims: list[int]):
...
def test_permute_wrong_rank(input_type: TensorType, dims: list[int]):
    rank = input_type.rank
...
def test_permute_duplicates(
    input_type: TensorType, dims: list[int], i: int, j: int
):
    assume(i != j)
    dims[i] = dims[j]
    with Graph("permute", input_types=[input_type]) as graph:
