    ...
    for gpu_idx in range(len(test_gpu_counts)):
        ...
        for dtype_idx in range(len(test_dtypes)):
            ...
            for length_idx in range(len(test_lengths)):
                ...
                print(
                    _get_test_str[dtype, use_multimem=False](num_gpus, length)
                )
                try:
                    # Test with use_multimem=false
                    allreduce_test[
                        dtype=dtype, rank=1, ngpus=num_gpus, use_multimem=False
                    ](ctx, length)
                except e:
                    if "OUT_OF_MEMORY" in String(e):
                        print(
                            "Out of memory error occurred for ",
                            _get_test_str[dtype, use_multimem=False](
                                num_gpus, length
                            ),
                        )
                    else:
                ...
                # Test with use_multimem=true
                print(_get_test_str[dtype, use_multimem=True](num_gpus, length))
                try:
                    allreduce_test[
                        dtype=dtype,
                        rank=1,
                    ...
                    if "OUT_OF_MEMORY" in String(e):
                        print(
                            "Out of memory error occurred for ",
                            _get_test_str[dtype, use_multimem=True](
                                num_gpus, length
                            ),
                        )
                    elif "multimem is only supported on SM90+ GPUs" in String(
                        e
                    ):
                        print(
                            "Skipping multimem test - SM90+ not supported"
