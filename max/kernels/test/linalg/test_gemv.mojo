...
def test_gemv():
    ...
    var lhs_storage = UnsafePointer[Scalar[type],].alloc[alignment=alignment](
        m * k
    )
    var lhs = NDBuffer[type, 2](lhs_storage, Index(m, k))

    var rhs_storage = UnsafePointer[Scalar[type],].alloc[alignment=alignment](k)
    var rhs = NDBuffer[type, 1, _, Dim(k)](rhs_storage)

    var out_storage = UnsafePointer[Scalar[type],].alloc[alignment=alignment](m)
