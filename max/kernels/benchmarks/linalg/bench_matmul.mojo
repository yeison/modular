...
fn bench_matmul[
    ...
    alias alignment = 64
    var a_ptr = UnsafePointer[Scalar[a_type], alignment2=alignment].alloc(
        spec.m * spec.k
    )
    var b_ptr = UnsafePointer[Scalar[b_type], alignment2=alignment].alloc(
        spec.k * spec.n
    )
    var c_ptr = UnsafePointer[Scalar[c_type], alignment2=alignment].alloc(
        spec.m * spec.n
    )
