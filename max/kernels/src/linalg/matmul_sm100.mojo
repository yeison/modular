...
fn store_C[
    ...
    *,
    ...
    for stage in range(num_stages):
        ...
        # if block_idx.x == 0 and block_idx.y == 0 and warp_id == 0 and elect_one_sync():
        #     print("stage_tmem_addr of warp 0", stage_tmem_addr | ((warp_id * 32) << 16), "stage_tmem_addr + (16 << 16)", stage_tmem_addr | ((warp_id * 32 + 16) << 16))
        # if block_idx.x == 0 and block_idx.y == 0 and warp_id ==1 and elect_one_sync():
        #     print("stage_tmem_addr of warp 1", stage_tmem_addr | ((warp_id * 32) << 16), "stage_tmem_addr + (16 << 16)", stage_tmem_addr | ((warp_id * 32 + 16) << 16))
        # if block_idx.x == 0 and block_idx.y == 0 and warp_id == 2 and elect_one_sync():
        #     print("stage_tmem_addr of warp 2", stage_tmem_addr | ((warp_id * 32) << 16), "stage_tmem_addr + (16 << 16)", stage_tmem_addr | ((warp_id * 32 + 16) << 16))
        # if block_idx.x == 0 and block_idx.y == 0 and warp_id == 3 and elect_one_sync():
        #     print("stage_tmem_addr of warp 3", stage_tmem_addr | ((warp_id * 32) << 16), "stage_tmem_addr + (16 << 16)", stage_tmem_addr | ((warp_id * 32 + 16) << 16))
        tcgen05_load_wait()
...
fn blackwell_matmul_tma_pair_mma[
    ...
    *,
    ...
    # NOTE: BM, 16 for 4096x4096x4096, 32 for rest. If changed, change swizzling to 32B as well
    alias output_tile_shape = Index(BM, 32)  # todo: ver
    ...
    alias tmem_writeout_smem = c_smem_bytes + tmem_addr_bytes
